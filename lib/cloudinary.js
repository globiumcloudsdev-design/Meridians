...
export const cloudinaryService = {
  ...
  // ============ PDF UPLOAD ============
  uploadPdf: async (file, folder = 'library', filename = 'document.pdf') => {
    try {
      return new Promise((resolve, reject) => {
        // Ensure the publicId does not contain spaces or weird characters, and strip .pdf as Cloudinary adds it automatically for 'auto'
        const cleanName = filename.replace(/\.pdf$/i, '').replace(/[^a-zA-Z0-9_-]/g, '_');
        const publicId = `doc_${Date.now()}_${cleanName}`;
