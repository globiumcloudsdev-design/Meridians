...
export default function TestResultScreen({ test, score, answers, onRetake, studentInfo }: TestResultScreenProps) {
        ...
        <div className="max-w-md mx-auto space-y-3">
          ...
          <Card className="rounded-xl border border-primary/20 overflow-hidden">
            ...
            <div className={`p-4 text-center ${
              ...
              <h2 className={`text-lg font-bold ${isPassed ? 'text-emerald-700' : 'text-red-700'}`}>
              ...
              <div className="mt-2 bg-white/80 rounded-lg p-1.5">
                <p className="text-[10px] text-muted-foreground">
                  Passing: <span className="font-semibold text-foreground">{test.passingMarks} ({((test.passingMarks / test.totalMarks) * 100).toFixed(0)}%)</span>
                </p>
              </div>
            </div>
            ...
            <div className="p-3 border-t border-border bg-muted/20">
              <div className="grid grid-cols-3 gap-2">
                <div className="bg-primary/5 rounded-lg p-1.5 text-center border border-primary/10">
                  <p className="text-base font-bold text-primary">{test.mcqs.length}</p>
                  <p className="text-[10px] text-muted-foreground">Total</p>
                </div>
                ...
                <div className="bg-amber-50 rounded-lg p-1.5 text-center border border-amber-200">
                  <p className="text-base font-bold text-amber-600">{test.mcqs.length - answeredCount}</p>
            ...
            <div className="p-3 border-t border-border">
              <h3 className="font-bold text-foreground flex items-center gap-1.5 mb-2 text-xs">
                    ...
                    >
                      <div className="flex items-start gap-1.5">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs text-foreground leading-snug line-clamp-2">{mcq.question}</p>
                          <div className="flex items-center gap-2 mt-0.5 text-[10px]">
                            <span className="text-muted-foreground">You:</span>
                            <span className={isCorrect ? 'text-emerald-600 font-medium' : answers[idx] !== undefined ? 'text-red-500 font-medium' : 'text-amber-600 font-medium'}>
                              {userAnswer}
                            </span>
                            {!isCorrect && (
                              <>
            ...
            <div className="p-3 border-t border-border bg-muted/20">
              <div className="flex gap-2">
                ...
                <Button
                  asChild
                  size="sm"
                  className={`h-9 bg-primary text-primary-foreground ${studentInfo ? 'w-full' : 'flex-1'}`}
                >
                  <Link href="/">
